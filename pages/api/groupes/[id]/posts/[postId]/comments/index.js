...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const data = { postId: parsedPostId, author: author || 'Anonymous', text };
    ...
    try {
      ...
      let postOwner = null;
      if (post.author) {
        postOwner = await prisma.user.findUnique({ where: { email: post.author } });
        if (!postOwner) {
          postOwner = await prisma.user.findUnique({ where: { nomUtilisateur: post.author } });
        }
      }
      if (postOwner) {
